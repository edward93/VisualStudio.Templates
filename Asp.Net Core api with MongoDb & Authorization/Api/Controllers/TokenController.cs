    ...
    public class TokenController : BaseController
    {
        ...
        public async Task<IActionResult> Post([FromForm] TokenViewModel model)
        {
            ...
            try
            {
                ...
                var result = await _userService.GetOne(filter.Eq(c => c.Email, model.UserName));
                if (result == null)
                {
                    throw new Exception("User name or password is incorrect");
                }

                bool isVerified = _userService.VerifyHashedPassword(result.PasswordHash, model.Password);
